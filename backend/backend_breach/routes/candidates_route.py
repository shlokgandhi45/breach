...
async def list_candidates(
    ...
    """
    query = db.query(Candidate).filter(Candidate.is_duplicate == 'false')
    ...
    if search and search.strip():
        ...
        query = (
            ...
            .filter(
                or_(
                    Candidate.full_name.op(like_op)(term) if not is_sqlite else Candidate.full_name.like(term),
                    ProfessionalInfo.current_job_title.op(like_op)(term) if not is_sqlite else ProfessionalInfo.current_job_title.like(term),
                )
            )
        )
...
async def get_candidate(
    ...
    # Ensure candidate_id is a UUID object for robust comparison
    try:
        id_obj = uuid.UUID(candidate_id)
    except ValueError:
        raise HTTPException(status_code=400, detail="Invalid UUID format.")

    candidate = db.query(Candidate).filter(
        Candidate.candidate_id == id_obj
    ).first()
