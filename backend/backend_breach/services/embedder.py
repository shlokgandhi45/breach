...
def _local_embedding(text: str) -> List[float]:
    ...
    if _local_model is None:
        try:
            ...
            # Fallback: instead of raising, return a zero vector and don't try again
            logger.warning("sentence-transformers not installed. Returning zero-vector fallback.")
            return [0.0] * 384

    # If model failed to load in a previous call
    if _local_model is None:
        return [0.0] * 384
