...
def generate_candidate_summary(
    ...
    try:
        ...
        if not settings.openai_api_key or "PROMPT_USER_FOR_KEY" in settings.openai_api_key:
            logger.warning(f"OpenAI key not configured. Using mock summary for {candidate_data.get('full_name')}.")
            match_reason = f"Candidate matches query '{recruiter_query}' based on their profile as a {candidate_data.get('job_title', 'professional')}."
            return {
                "summary":            match_reason,
                "query_match_reason": match_reason,
                "matched_skills":     candidate_data.get("skills", [])[:3],
                "skill_gaps":         [],
                "experience_years":   f"{candidate_data.get('experience_years', 'N/A')} years",
                "current_role":       f"{candidate_data.get('job_title', 'Unknown')} at {candidate_data.get('company', 'Unknown')}",
                "confidence":         "medium",
            }
