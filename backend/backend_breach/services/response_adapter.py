...
def adapt_candidate(
    ...
    """
    def _compute_mock_score(name: str, exp_years: int, has_edu: bool) -> int:
        w = ai_config.get_weights()
        hash_val = int(hashlib.md5(name.encode()).hexdigest(), 16)
        
        seed_skill = (hash_val % 41) + 55 
        seed_exp = 100 if exp_years >= 5 else (exp_years * 20)
        seed_culture = ((hash_val // 10) % 41) + 55
        seed_edu = 100 if has_edu else 50
        
        total_w = max(w.skill_weight + w.experience_weight + w.culture_fit_weight + w.education_weight, 1.0)
        
        final = (
            (seed_skill * w.skill_weight) +
            (seed_exp * w.experience_weight) +
            (seed_culture * w.culture_fit_weight) +
            (seed_edu * w.education_weight)
        ) / total_w
        
        return min(int(round(final)), 99)
    ...
    return {
        ...
        # Match & pipeline
        'matchScore':       _compute_mock_score(name, exp_years or 0, bool(education_str)), # AI Config Weighted
        ...
        # Compensation (Mock generation based on experience)
        'salary':           f"₹{((exp_years or 2) * 2.5) + 6:g}L - ₹{((exp_years or 2) * 3) + 8:g}L" if exp_years is not None else "₹8L - ₹12L",
        'noticePeriod':     "30 Days" if (exp_years or 0) < 5 else "60 Days" if (exp_years or 0) < 8 else "90 Days",
