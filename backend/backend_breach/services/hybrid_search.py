...
def bm25_search(
    ...
    if is_sqlite:
        ...
        # Robust SQLite fallback using ORM
        query_obj = db.query(Resume.candidate_id)
        if candidate_filter_ids:
            # Convert strings to UUID objects if needed
            import uuid
            id_objs = []
            for cid in candidate_filter_ids:
                try: id_objs.append(uuid.UUID(cid))
                except: id_objs.append(cid)
            query_obj = query_obj.filter(Resume.candidate_id.in_(id_objs))
        
        # Build LIKE filters for each term — use ilike for case-insensitivity on SQLite
        filters = [Resume.resume_text.ilike(f"%{t}%") for t in terms]
        if filters:
            query_obj = query_obj.filter(or_(*filters))
        
        rows = query_obj.limit(top_k).all()
        logger.info(f"BM25 SQLite fallback: terms={terms}, found {len(rows)} candidates")
        return [(str(row.candidate_id), 1.0) for row in rows]
    else:
        # Postgres Full Text Search (unchanged raw SQL for performance)
