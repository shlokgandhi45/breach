...
class GoogleSheetsService:
    ...
    def remove_candidate(self, candidate_id: str) -> bool:
        """
        Removes a candidate from the Google Sheet by ID.
        """
        if not self.webapp_url:
            return True
        ...
        try:
            ...
            logger.info(f"Removing candidate ID {candidate_id} from Google Sheets...")
            ...
            logger.error(f"Error removing from Google Sheets: {str(e)}")
            return False
