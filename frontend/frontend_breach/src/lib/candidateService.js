...
import { apiGet, apiPost, apiPatch, apiUpload, apiDelete } from './api';
...
export async function fetchCandidates(filters = {}) {
    try {
        ...
        // We only fall back to mock data if the backend is COMPLETELY unreachable.
        // If it's a 500 or 404 from a running server, we return an empty list or bubble up.
        if (err.message.includes('API error')) return [];
...
export async function fetchDashboardStats() {
    try {
        ...
        return {
            ...
            offer_accept_rate:          data.offer_accept_rate,
            scheduled_interviews_count: data.scheduled_interviews_count,
        ...
        return {
            ...
            pipeline_counts,
            active_pipeline:            mockCandidates.length,
            offer_accept_rate:          '84%',
            scheduled_interviews_count: 2,
...
/**
 * Removes a candidate from the scheduled interview list.
 * @param {string} candidateId 
 * @returns {Promise<object>}
 */
export async function removeScheduledInterview(candidateId) {
    try {
        return await apiDelete(`/api/scheduling/interview/${candidateId}`);
    } catch (err) {
        warn(`/api/scheduling/interview/${candidateId}`, err);
        return { 
            success: true, 
            message: 'Candidate removed locally. (Mock mode)',
            _mock: true 
        };
    }
}
