...
export default function SearchOverlay({ open, onClose }) {
    ...
    async function handleSearch(e, altQuery = null) {
        if (e) e.preventDefault();
        const searchQuery = altQuery || query;
        if (!searchQuery.trim()) return;
        setLoading(true);
        const data = await searchCandidates(searchQuery.trim());
                                ...
                                {jobRoles.length > 0 ? (
                                    jobRoles.map(role => (
                                        <button
                                            key={role.role_id}
                                            onClick={() => {
                                                setQuery(role.role_title);
                                                handleSearch(null, role.role_title);
                                            }}
